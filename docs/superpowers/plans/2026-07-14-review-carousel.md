...
使用 5000ms 定时器切换下一条；轮播 `mouseenter`、`focusin` 和 `document.visibilitychange` 停止计时，`mouseleave`、`focusout` 且焦点离开容器时恢复。减少动态效果匹配时不启动计时。
...
确认一次只显示一张完整评价、按钮和圆点可用、5 秒后自动切换、悬停暂停且页面无横向溢出。
...
确认键盘左右键、焦点暂停、当前圆点状态、手动切换播报、减少动态效果规则、控制台无错误，并复测导航和预约表单关键逻辑。
